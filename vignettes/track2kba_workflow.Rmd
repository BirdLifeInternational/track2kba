...
```{r example, eval=F, message=FALSE, warning = FALSE, include=T}
...
```{r eval=F, warning = FALSE, message=FALSE, include=T, fig.height=7, fig.width=7}
trips <- tripSplit(
  tracks     = tracks, 
  Colony     = colony, 
...
knitr::include_graphics("man/figures/README-unnamed-chunk-3-1.png", dpi=50)
```
...
```{r estSpaceUse, eval=F, warning = FALSE, message = FALSE, include = TRUE, fig.width=5, fig.height=4, dpi=300}
...
knitr::include_graphics("man/figures/README-estSpaceUse-1.png", dpi=50)
```
...
```{r repAssess, eval=FALSE, echo=TRUE}
...
```{r repAssess graphic, echo=FALSE, fig.align="center", out.width="100%"}
knitr::include_graphics("track2kba_repAssess_output.png", dpi=1000)
```
...
```{r findKBA, eval=F, fig.width=5, fig.height=4, fig.align="center", dpi=300}
KBAs <- findKBA(
  KDE = KDEs,
  Represent = repr$out,
  UDLev = 50,
...
```
```{r boobies graphic1, echo=FALSE}
knitr::include_graphics("KBA_sf_plot.png", dpi=200)
```
