...
Now, using `findSite` we can identify aggregation areas. Using the core area estimates of each individual we can calculate where they overlap. Then, we estimate the proportion of the larger population in a given area by adjusting our overlap estimate based on the degree of representativeness.
...
If we specified `polyOut=TRUE`, then the output will be in Simple Features format, which allows us to easily take advantage of the `ggplot2` plotting syntax to make an attractive map using `mapSite`!
...
Then, we can combine all the polygons within the 'potentialSite' area, and using the maximum number of individuals present in that area we can assess whether it merits identification as a Key Biodiversity Area according to the KBA standard.
...
If in `findSite` we instead specify `polyOut=FALSE`, our output will be a spatial grid of animal densities, with each cell representing the estimated number, or percentage of animals using that area. So this output is irrespective of the representativness-based importance threshold.
