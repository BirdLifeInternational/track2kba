...
#' @param Res numeric. Grid cell resolution (in square kilometers) for Kernel Density Estimation. Default is a grid of 500 cells, with spatial extent determined by the data.
...
#' @return A tibble dataframe, with columns '\emph{SampleSize}' signifying the max sample size, '\emph{out}' signifying the percent representativeness of the sample. '\emph{type}' is the type of asymptote value used to calculate the '\emph{out}' value, and '\emph{asym}' is the asymptote value used.
#'
