...
#'
#' @param tracks data.frame or SpatialPointsDataFrame. Must have an 'ID' field; if input is data.frame or unprojected SpatialPointsDF, must also include 'Latitude' and 'Longitude' fields. For formatting issues see \code{\link{formatFields}}.
...
#' @param levelUD numeric (percent). Specify which utilization distribution contour to show in the plotted output. NOTE: this will only affect the output if \code{polyOut=TRUE}.
...
#' @param polyOut logical scalar (TRUE/FALSE). If TRUE then output will include a plot of individual UD polygons and a simple feature with kernel UD polygons for the level of \code{levelUD}. NOTE: creating polygons of UD is both computationally slow and prone to errors if the usage included in \code{levelUD} extends beyond the specified grid. In this case \code{estSpaceUse} will return only the \code{estUDm}object and issue a warning.
...
#'
#' If \code{polyOut=TRUE} the output will be a list with two components: \emph{'KDE.Surface'} is the \code{estUDm} object and \code{UDPolygons} is polygon object of class \code{sf} (Simple Features) with the UD contour for each individual at the specified \code{levelUD}.
#'
...
estSpaceUse <- function(tracks, scale = 50, levelUD = 50, res=NULL, polyOut=FALSE) {
  ...
  tracks@data$ID<-droplevels(as.factor(tracks@data$ID))        ### encountered weird error when unused levels were retained (27 Feb 2017)
  ...
  xrange <- seq(minX,maxX, by = res*1000) #diff(range(coordinates(tracks)[,1]))/res)   ### if res should be provided in km we need to change this
  yrange <- seq(minY,maxY, by = res*1000) #diff(range(coordinates(tracks)[,2]))/res)   ### if res should be provided in km we need to change this
  grid.locs <- expand.grid(x=xrange,y=yrange)
  INPUTgrid <- SpatialPixels(SpatialPoints(grid.locs), proj4string=proj4string(tracks))
  #  plot(INPUTgrid)
